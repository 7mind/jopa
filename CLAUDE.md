...
## JDK Compliance Testing Tool

A python script is available at `scripts/compliance_tester.py` to facilitate running JDK compliance tests against the JOPA compiler.
...
- Supports blacklisting flaky or irrelevant tests.
...
- `--testlist <output_file>`: Optional. Path to write the generated whitelist (default: `test_whitelist.txt`).

**Example:**
```bash
# Prepare tests for JDK 7, writing to jdk7_tests.txt
./scripts/compliance_tester.py --prepare --jdk 7 --testlist jdk7_tests.txt
```
...
- `--testlist <input_file>`: Path to the file containing the list of tests to run (generated by `--prepare`).
- `--jdk {7,8}`: Use to scan and run directly without an intermediate list file.
...
**Example:**
```bash
# Run 50 random tests from the generated list with JOPA
./scripts/compliance_tester.py --test --testlist jdk7_tests.txt --limit 50 --compiler ./build/src/jopa --timeout 10 --no-success --verbose-failures
```
